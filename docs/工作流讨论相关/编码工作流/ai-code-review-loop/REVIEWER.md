...
| 识别正确性、边界、错误处理、安全、事务、幂等、并发、资源、性能、可维护性和观测性风险 | 编写 Playwright、UI E2E 或跨系统验收测试 |
...
- 如为 Java 代码，默认读取当前 skill 附带的 `JAVA_REVIEW_CHECKLIST.md`。
- 如变更涉及并发、锁、线程池、异步、共享状态、缓存一致性或高频热点路径，再读取当前 skill 附带的 `CONCURRENCY_REVIEW_CHECKLIST.md`。
...
6. **工程风险**：检查事务、幂等、并发、资源释放、安全、观测性、兼容、性能和缓存一致性风险。
7. **可维护性与质量红线**：检查命名、注释、重复代码、抽象层次、扩展成本，以及阿里巴巴开发规约 / SonarQube 红线所暴露的真实风险。
8. **回归风险**：检查是否破坏现有行为、公共接口、数据契约或测试约定。
9. **给出结论**：只报告有证据的问题，按固定格式输出 verdict。
...
- 存在明确安全漏洞或质量红线问题，例如 SQL 注入、XSS、命令注入、危险反序列化、敏感信息泄露或同级别高风险缺陷。
- 事务、幂等、并发或资源生命周期存在会导致数据错误、重复执行、死锁、泄漏或不可恢复失败的风险。
- 锁粒度、锁顺序、长临界区、缓存策略或热点路径实现明显不合理，已足以导致正确性错误、严重竞争、尾延迟失控或资源失控。
- 测试只验证 mock 交互、调用次数或实现细节，无法证明批准用例要求的真实行为；或关键契约仅靠单测无法证明却缺少必要的单模块接口集成测试证据。
...
- 测试是否验证可观察业务行为，而不只是 mock 交互、日志文本、私有实现细节或脆弱的调用顺序。
- 边界条件、异常路径和关键状态流转是否被覆盖。
- 对外契约、持久化、副作用编排或框架集成风险较高时，是否提供了相应的单模块接口集成测试证据。
- Refactor 后是否重跑相关测试。
...
- 事务边界是否覆盖必须原子提交的操作。
- 锁粒度、锁顺序、锁内操作和等待策略是否合理，是否存在长临界区、嵌套锁、睡眠轮询或共享状态保护不一致。
- 幂等逻辑是否能处理重复请求、重试或并发触发。
- 并发访问是否保护共享可变状态。
- 缓存是否有明确的一致性、失效、容量和并发访问策略，是否可能产生脏读、雪崩、穿透或无界增长。
- I/O、连接、流、线程池、锁等资源是否有明确生命周期和释放路径。
- 日志和指标是否足以排查关键失败，且不泄露敏感信息。
- 新增依赖、配置或迁移是否有必要性和可追溯依据。
- 热点路径、集合处理、对象分配、线程池与队列容量、阻塞调用位置是否存在明显性能瓶颈或尾延迟风险。
- 实现是否过度复杂，是否存在重复代码、命名失真、缺少必要注释、过早抽象或隐藏行为分支，导致明显维护风险。

### 安全与质量红线

检查：

- 用户输入、外部报文、SQL/HQL/脚本片段、模板输出和序列化边界是否存在注入、越权、XSS、危险反序列化或同级别高风险问题。
- 错误日志、审计日志、指标标签和异常消息是否泄露密码、token、密钥、证件号、卡号或其他敏感数据。
- 是否触犯阿里巴巴开发规约或 SonarQube 质量红线中与正确性、资源泄露、并发安全、安全漏洞、复杂度失控直接相关的条目。
- 纯格式或风格问题不要阻塞；只有会导致真实风险的规约/红线问题才列为 blocking issue。

## 附加 Checklist 使用规则

如果项目提供语言或领域 checklist：

- Java 代码默认读取当前 skill 自带的 `JAVA_REVIEW_CHECKLIST.md`。
- 涉及并发、锁、线程池、异步、共享状态、缓存一致性或热点路径时，再读取当前 skill 自带的 `CONCURRENCY_REVIEW_CHECKLIST.md`。
