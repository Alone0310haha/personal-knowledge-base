...
写 Architecture Design Doc 时，尽可能使用业务语义描述：

- 优先写业务对象、业务动作、业务状态、业务规则和业务结果，例如"订单状态变更后触发成交回写"，不要只写"调用 service 方法"。
- 组件、接口、数据流和错误处理都要说明它们承载的业务职责，而不是只罗列技术名词、类名、表名或框架机制。
- 技术术语只在澄清边界、约束或实现不可替代时使用，并绑定到对应业务语义。
- 如果只能用技术名词描述某个设计点，先补一句它解决的业务问题或保护的业务不变量。
...
## Summary
{3-5 句：本设计解决什么业务问题、采用什么架构方向、主要业务边界和非目标}
...
- **业务语义清晰**：design 是否尽可能用业务对象、业务动作、业务状态和业务规则描述？技术名词是否绑定到对应业务职责？
