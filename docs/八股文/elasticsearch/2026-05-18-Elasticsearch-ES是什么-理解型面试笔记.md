...
```mermaid
flowchart LR
  C[Client / Query DSL] --> IDX

  subgraph LOGIC[逻辑模型：面向业务与查询]
    IDX[Index：一类文档的逻辑集合]
    MAP[Mapping：字段类型与索引规则]
    DOC[Document：一条 JSON 数据]
    FIELD[Field：JSON 字段]
    TEXT[text 字段]
    KW[keyword/date/number 字段]
    ANA[Analyzer：分词 + 归一化]
  ...
  subgraph PHYSICAL[物理分布：面向存储与并行]
    CL[Cluster：集群]

    subgraph N1[Node A：ES 进程]
      P0[Primary Shard 0]
      R1[Replica Shard 1]
    end

    subgraph N2[Node B：ES 进程]
      P1[Primary Shard 1]
      R0[Replica Shard 0]
    end
    ...
    L0 --> S00[Segment A：不可变数据段]
    L0 --> S01[Segment B：不可变数据段]
    L1 --> S10[Segment]
    L2 --> S20[Segment]
    L3 --> S30[Segment]
  end

  subgraph SEG[Segment 内部：一批文档生成的 Lucene 数据]
    SDOC[多条 Document]
    SFIELD[多个 Field 的索引数据]
    INV[倒排索引：term -> docIDs]
    DV[Doc Values：列式值，用于排序/聚合]
    STORED[Stored Fields / _source]
    DEL[删除标记]
  ...
  IDX -.被拆分成 primary shards.-> P0
  IDX -.被拆分成 primary shards.-> P1
  P0 -.复制.-> R0
  P1 -.复制.-> R1
  CL --> N1
  CL --> N2
  S00 --> SEG
  MAP -.指导字段如何写入索引.-> SFIELD
```

这几个概念要分成“逻辑模型”和“物理模型”两条线理解：Index 是面向业务和查询的逻辑集合，Node 是 ES 进程，Shard 是 Index 真正被切分后的物理分布单位，Segment 是 Shard 内部 Lucene 生成的不可变数据段。

- Index 不直接存放 Segment。一个 Index 会先被拆成多个 Primary Shard，每个 Primary Shard 可以再复制出 Replica Shard；查询 Index 时，ES 实际会把请求路由到相关 Shard 上并行执行。
- Node 也不是按 Index 做内部划分。一个 Node 上承载的是多个 Shard copy，这些 Shard copy 可以来自同一个 Index，也可以来自不同 Index；Cluster 负责把 Shard 分配到不同 Node 上。
- Shard 是连接逻辑和物理的关键。对上看，它属于某个 Index；对下看，每个 Shard 本质上是一个 Lucene Index，内部由多个 Segment 组成。
- Segment 是 Lucene 的不可变数据段。一个 Segment 里包含一批 Document，以及这些 Document 的多个 Field 生成出的倒排索引、Doc Values、Stored Fields / `_source`、删除标记等数据。
- Mapping 是 Index 级别的字段规则，不是 Segment 的子结构。写入 Document 时，ES 会根据 Mapping 判断 Field 类型；text 字段会经过 Analyzer 分词后建立倒排索引，keyword/date/number 等字段会按精确匹配、排序、聚合等场景生成对应的索引结构。
